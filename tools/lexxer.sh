...
do_parallel=0     # включить GNU Parallel. ВНИМАНИЕ: подобрать параметры по реальной производительности
   par_dixa=0     # включить GNU Parallel для применения словаря ударений.
...
# Список обычных слов (только с маленькой буквы) -- слова, отсутствующие в словарях ударений
 sed -r 's/^/ /g' $stadir/text-book.txt | grep -Po "(?<![$RUUC$rulc$unxc])[$rulc$unxc]+" | grep -Ev "[$unxc]" | sed -r "s/^.+$/_\0=/g" | \
  grep -Fvf <(zcat $stadir/unistress-all.pat.gz $stadir/mano-sm.pat.gz $stadir/malc.pat.gz) | sort -u | sedlist > 01_wrd_newuni.list

# Список слов с заглавной буквы НЕ в начале предложения
grep -Po "(?<=[A-Za-zА-Яа-яёЁ0-9,}):;$unxc$unxn])[ ’(«$qtsd–—-]+[$unxc]?[$RUUC][$rulc$unxc]+" $stadir/text-book.txt | grep -Ev "[$unxc]" | \
...
LC_ALL="en_US.UTF-8" printf '\e[36m%s \e[93m%.2f \e[36m%s, \e[0m' "Списки слов:" $duration "сек"
 ...
 touch "$stadir"/anycap-lex.pat "$stadir"/anycap-bas.pat "$stadir"/surcap-lex.pat "$stadir"/surcap-bas.pat;
...
LC_ALL="en_US.UTF-8" printf '\e[36m%s \e[93m%.2f \e[36m%s, \e[0m' "Слов:" $duration "сек"
 ...
 grep -Ff <(zcat $stadir/namebase.pat.gz) 01_wrd_newuni.list > $stadir/06_nom_inlc.pat
 grep -Ff <(zcat $stadir/mano-sm.pat.gz)  01_wrd_newuni.list > $stadir/07_nom_omlc.pat
#uniz="$stadir/unistress.gz $stadir/yoyo.gz $stadir/yodef.gz"
...
if [[ $dixa -eq 1 ]]; then # Применить все словари
  ...
  if [[ $do_parallel -eq 1 && $par_dixa -eq 1 ]]; then
    parallel --env $paraopts_awk "$stadir"/text-book.txt awk -v indb="$sdb/" -v inax="$stadir" -f $sdb/unidef.awk > "$wrkdir"/text-book.txt ;
  else
...
printf '\e[33m%s \e[93m%-6s \e[33m%s \e[93m%-6s \e[32m%s\e[0m\n' \
  "00_WRDs лексика :" $(grep -c ^ 00_wrd_newlex.list) "01_UNIs ударения:" $(grep -c ^ 01_wrd_newuni.list) "<== Новые слова"
printf '\e[33m%s \e[93m%-6s \e[33m%s \e[93m%-6s \e[32m%s\e[0m\n' \
