...
morphy_is=3   # 1 = SpaCy; 2 = Natasha; 3 = beta_1; NOTE: только для исследовательских целей. На практике используем ruaccent.
...
if [[ $fixomo == "1" ]]; then
   ...
   # Создать копию текст книги и морфологией с помощью morphy << начало блока morphy
     ...
     if [[ $morphy_is == "1" ]]; then
       ...
       if [[ -s $bookstadir/text-book.scy ]] && md5sum -c --status $bookstadir/text.shy.md5 >/dev/null 2>&1; then
          ...
          md5sum $bookstadir/text-book.scy $bookwrkdir/text-book.txt $sdb/rulg_omo.py $sdb/rulg_all.py > $bookstadir/text.shy.md5
          mo_cur=$(date +%s.%N); duration=$( echo $mo_cur - $mo_prev | bc ); mo_prev=$mo_cur; durhum=$(ms2sec);
    ...
    if [[ $morphy_is == "2" ]]; then
       ...
       if [[ -s $bookstadir/text-book.nat ]] && md5sum -c --status $bookstadir/text.nhy.md5 >/dev/null 2>&1; then
          ...
          md5sum $bookstadir/text-book.nat $bookwrkdir/text-book.txt $sdb/natru_omo.py > $bookstadir/text.nhy.md5
          mo_cur=$(date +%s.%N); duration=$( echo $mo_cur - $mo_prev | bc ); mo_prev=$mo_cur; durhum=$(ms2sec);
  ...
  case $morphy_is in
    ...
    3) if [[ -s $bookstadir/text-book.bet ]]; then
         awk '{if (FNR==NR) { a[FNR]=$0 } else { b[FNR]=$0 } }; END { for(i in a) print a[i] "<@##@##@>" b[i] }' \
         $bookwrkdir/text-book.txt $bookstadir/text-book.bet > $bookwrkdir/text-book.bas
       else
         cp -fu $bookwrkdir/text-book.txt $bookwrkdir/text-book.bas
       fi; ;;
     *)  cp -fu $bookwrkdir/text-book.txt $bookwrkdir/text-book.bas ;;
...
#             s/^(.+#_#_# vsez !_#_!)$/#\1/g;
      ...
      awk -vindb="$sdb/" -vinax="$aux/" -vbkphydir="$bookstadir/" -vlocdic="$bookstadir/" -vmorphy_on="$morphy" -vmorphy_yo="$morphy_yo" \
          -vescan="$eSCAN" -vescap="$eSCAP" -vnoredix=1 -vvso="$vso" -f $bookstadir/main_esc.awk $bookwrkdir/text-book.bas > $bookwrkdir/text-book.awk.txt
...
if [[ $rust -eq 2 ]]; then # Обработать текст silero-stress’ом
  ...
  python $sdb/sist.py $ruac_opt $bookwrkdir/text-book.txt > $bookwrkdir/text-book.sist.txt
