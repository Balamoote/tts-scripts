...
# Использование: diff эталонный_корпус.txt результат_разметки.txt | awk -f korpus_diff.awk
# Конфигурация омографа задаётся в блоке CONFIG ниже

BEGIN {
    # ====== CONFIG: меняйте только здесь ======
    base = "все"           # неразмеченная форма (нижний регистр)
    forms[1] = "все́"       # размеченная форма 1 (нижний регистр)
    forms[2] = "всё"       # размеченная форма 2 (нижний регистр)
#   forms[3] = "всё"       # размеченная форма 3 (нижний регистр)
