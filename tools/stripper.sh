...
case $key in 
  ...
  -w) # удалить служебные символы и ударения в указанном слове, регистрозависимо. Бэкап НЕ делать. Только 1 вариантов: Слова|СЛОВА|слова
  	  if [[ -n $3 ]]; then
           b2t; somo=$3; swch="ws"; printf '\e[36m%s \e[93m%s\e[0m ' "Очистить слово" $somo
           awk -vwomo=$somo -vindb=$indb -vswch=$swch -f $sdb/awx/stripper.awk trip-"$book"/text-book.txt > trip-"$book"/text-book.txt.awk
  ...
  -wb) # удалить служебные символы и ударения в указанном слове, регистрозависимо. Сделать бэкап.
  	  if [[ -n $3 ]]; then
           d2u; b2t; somo=$3; swch="ws"; printf '\e[36m%s \e[93m%s\e[0m\n' "Очистить слово" $somo
