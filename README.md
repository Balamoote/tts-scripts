...
Словари ударений tts-scripts и ruaccent’a в общем случае не совпадают и имеет смысл выполнить ./lexxer.sh -dixa book.fb2 до обработки ruaccent’ом, т.к. скорее всего
...
4. Обработка омографов в ручном режиме.

4. 1. Программа omograph-tk.py предоставляет графический интерфейс для работы. Для начала работы нужно выбрать одну из папок с дискретными словами.
Настройки находятся в .omograph-gui.json

4. 2. Как альтернатива, можно работать с дискретными скриптами непосредственно в директориях mano- namo- yoy- в зависимости от того, на каком этапе
обработки вы находитесь.

После этого можно сразу использовать пакет ruaccent, не тратя время на ручную обработку.

5. ./momo.sh -ruac book.fb2 = выполнить автообработку всех слов текста, включая омографы, и ёфикацию. Особенность: уже проставленные ударения остаются на месте.</br>

Настройку работы ruaccent можно сделать в файле scripts/ruac.py</br>

6. Скрипт ./stripper.sh позволяет откатить либо все изменения или только некоторые из них (тут ещё не доделана очистка группы).

7. Скрипт ./fixxer.sh в полностью обработанный текст вносит правки, которые исправляют произношение конкретного tts-движка.
